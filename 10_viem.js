...
async function getNftOwner( tokenId) {
    try {
        ...
        // 调用 ownerOf 函数，获取持有人地址
        const owner = await contract.read.ownerOf([tokenId])
        console.log(`Owner of tokenId ${tokenId}: ${owner}`);

        // 调用 tokenURI 函数，获取元数据 URI
        const url =await contract.read.tokenURI([tokenId])
        console.log(`Metadata URI of tokenId ${tokenId}: ${url}`);
    } catch (error) {
        console.error(`Failed to fetch owner or metadata for tokenId ${tokenId}:`, error);
    }
}
